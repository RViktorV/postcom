...
def validate_age(author):
    ...
    if author.date_of_birth:  # Предполагается, что у пользователя есть поле date_of_birth
        age = timezone.now().year - author.date_of_birth.year
        if (timezone.now().month, timezone.now().day) < (author.date_of_birth.month, author.date_of_birth.day):
            age -= 1
        if age < 18:
