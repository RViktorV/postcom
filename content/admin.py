...
class PostAdmin(admin.ModelAdmin):
    list_display = (
        ...
        "author",
        "created_at",
    )  # Отображаем заголовок, автора и дату создания
    list_filter = ("created_at",)  # Добавляем фильтр по дате создания
    search_fields = (
        "title",
        "text",
    )  # Добавляем возможность поиска по заголовку и содержимому

    def created_at(self, obj):
        return obj.created_at  # Используем поле created_at для отображения

    created_at.admin_order_field = (
        "created_at"  # Позволяет сортировать по дате создания
    )
...
class CommentAdmin(admin.ModelAdmin):
    list_display = (
        ...
        "author",
        "created_at",
    )  # Отображаем пост, автора и дату создания
    list_filter = ("created_at",)  # Добавляем фильтр по дате создания
